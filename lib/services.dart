...
void onStart(ServiceInstance service) async {
  ...
  Future<bool> _attemptLocationUpdate(ServiceInstance service, bool currentRetryMode, int currentRetryAttempts) async {
  ...
  startRetryTimer = () {
    retryTimer?.cancel();
    retryTimer = Timer.periodic(const Duration(minutes: 5), (timer) async {
      if (isRetryMode && retryAttempts < maxRetryAttempts) {
        bool success = await _attemptLocationUpdate(service, isRetryMode, retryAttempts);
        ...
        if (success) {
          ...
          if (retryAttempts >= maxRetryAttempts) {
            print("🛑 Max retry attempts reached - Waiting for next main cycle");
            isRetryMode = false;
            retryAttempts = 0;
            retryTimer?.cancel();
          }
  ...
  void startMainTimer() {
    mainTimer?.cancel();
    mainTimer = Timer.periodic(const Duration(hours: 6), (timer) async {
      if (!isRetryMode) {
...
/// تنفيذ تحديث الموقع (إرجاع true إذا نجح، false إذا فشل)
Future<bool> _performLocationUpdate(ServiceInstance service) async {
  try {
    ...
    if (success) {
      ...
      await prefs.setDouble('last_lat', position.latitude);
      await prefs.setDouble('last_lng', position.longitude);
      await prefs.setString('last_successful_update', DateTime.now().toIso8601String());
